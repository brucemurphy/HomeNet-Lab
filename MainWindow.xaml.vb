...
Class MainWindow
    ...
    ' Latest Bing image info (shared with generated web portal)
    Private _bingImageUrl As String = ""
    Private _bingHeadline As String = ""
    Private _bingCopyright As String = ""
    ...
    Private Async Function SetBingWallpaperAsync(Optional ct As CancellationToken = Nothing) As Task
        ...
        ' Cache the Bing image info so the generated web portal can mirror
        ' the main app's look-and-feel (same wallpaper, headline, copyright).
        _bingImageUrl = fullImageUrl
        _bingHeadline = If(String.IsNullOrWhiteSpace(headline), "", headline)
        _bingCopyright = If(String.IsNullOrWhiteSpace(copyright), "", copyright)
    ...
    Private Function GenerateWebPortalHtml() As String
        ...
        If Not String.IsNullOrEmpty(_bingImageUrl) Then
            html.AppendLine($"            background: #000 url('{_bingImageUrl}') center center / cover no-repeat fixed;")
        Else
            html.AppendLine("            background: linear-gradient(135deg, #1a1a1a 0%, #2a2a2a 100%);")
        End If
        ...
        html.AppendLine("            color: #fafafa;")
        html.AppendLine("        }")
        html.AppendLine("        .container {")
        html.AppendLine("            background: rgba(0, 0, 0, 0.75);")
        html.AppendLine("            border-radius: 10px;")
        html.AppendLine("            box-shadow: 0 20px 60px rgba(0, 0, 0, 0.6);")
        html.AppendLine("            padding: 40px;")
        html.AppendLine("            max-width: 600px;")
        html.AppendLine("            width: 100%;")
        html.AppendLine("            backdrop-filter: blur(14px);")
        html.AppendLine("            -webkit-backdrop-filter: blur(14px);")
        ...
        html.AppendLine("            background: rgba(30, 30, 30, 0.95);")
        html.AppendLine("            color: #fafafa;")
        ...
        html.AppendLine("        .bing-credit {")
        html.AppendLine("            position: fixed;")
        html.AppendLine("            left: 20px;")
        html.AppendLine("            bottom: 20px;")
        html.AppendLine("            max-width: 45%;")
        html.AppendLine("            padding: 10px 16px;")
        html.AppendLine("            background: rgba(0, 0, 0, 0.55);")
        html.AppendLine("            border-radius: 8px;")
        html.AppendLine("            color: #ffffff;")
        html.AppendLine("            backdrop-filter: blur(6px);")
        html.AppendLine("            -webkit-backdrop-filter: blur(6px);")
        html.AppendLine("            pointer-events: none;")
        html.AppendLine("        }")
        html.AppendLine("        .bing-credit .headline {")
        html.AppendLine("            font-size: 1.1em;")
        html.AppendLine("            font-weight: 600;")
        html.AppendLine("            margin-bottom: 2px;")
        html.AppendLine("        }")
        html.AppendLine("        .bing-credit .copyright {")
        html.AppendLine("            font-size: 0.8em;")
        html.AppendLine("            color: #cccccc;")
        html.AppendLine("        }")
        html.AppendLine("        @media (max-width: 700px) { .bing-credit { display: none; } }")
        html.AppendLine("    </style>")
        html.AppendLine("</head>")
        html.AppendLine("<body>")
        ...
        html.AppendLine("    </div>")
        html.AppendLine("")
        If Not String.IsNullOrEmpty(_bingHeadline) OrElse Not String.IsNullOrEmpty(_bingCopyright) Then
            html.AppendLine("    <div class=""bing-credit"">")
            If Not String.IsNullOrEmpty(_bingHeadline) Then
                html.AppendLine($"        <div class=""headline"">{System.Net.WebUtility.HtmlEncode(_bingHeadline)}</div>")
            End If
            If Not String.IsNullOrEmpty(_bingCopyright) Then
                html.AppendLine($"        <div class=""copyright"">{System.Net.WebUtility.HtmlEncode(_bingCopyright)}</div>")
            End If
            html.AppendLine("    </div>")
            html.AppendLine("")
        End If
